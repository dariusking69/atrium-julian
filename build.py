...
# The Julian only — AppFolio's saved property list scopes this server-side.
# 480 N Terry Ave, Orlando FL 32801 (single building).
LISTINGS_URL = f"{BASE}/listings?filters%5Bproperty_list%5D=The+Julian"
...
def fetch_grid():
    ...
    while page <= 50:  # safety bound
        sep = "&" if "?" in LISTINGS_URL else "?"   # LISTINGS_URL may already carry a filter query
        url = LISTINGS_URL if page == 1 else f"{LISTINGS_URL}{sep}page={page}"
...
def build(listings):
    for l in listings:
        ...
        # Single-building property: every unit shares one street address, so surface the unit no.
...
# ===================== TEMPLATES =====================
SHARED_CSS = r"""
  :root{--red:#003b4a;--red-dk:#00262f;--ink:#121212;--ink-2:#424245;--line:#e6e6e6;
